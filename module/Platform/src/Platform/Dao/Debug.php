...
class  Debug extends BaseDao
{
	public function getLogList($param,$offset,$limit){
	    $db =new Mon();
        return $db->setDb('common_mongo')->setCollection('error_log')->getList($param,array(),$offset,$limit,array('ctime'=>-1));
	}

    public function getLogNum($param){
        $db =new Mon();
        return $db->setDb('common_mongo')->setCollection('error_log')->getNum($param);
    }

    public function delete($data){
        $db = new Mon();
        return $db->setDb('common_mongo')->setCollection('error_log')->delete($data);
    }
